...
All the HTML5 templates are in the at_core/templates/ directory - if you need
...
page_layout plugins and Panels module plugins. Both are responsive and can be
configured via the theme settings on your themes appearace configuration page.
...
layouts will load automatically inside Panels - they are called AT Responsive
Panels. Only AT Responsive Panels are responsive, the normal Panels layout or
other panels layouts may not be and will not respond to the settings.
...
away in a group of settings called "Extensions". To enable these extensions
toggle the setting in the "Settings" tab. There are additional features and
options for Fonts, Heading and title styles, Image alignment and captions,
breadcrumbs, login block setting, mobile metatags, Apple touch icons, many
options for overrideing markup and even a way to enable a page.tpl.php template
that uses 100% width wrappers for designs that need this. Additionally there is
an extension that allows end users to paste in CSS - you don't need to create
an entire sub-theme to make small changes.
